...
Detailed logging like original rule_creator_gui.py
"""
...
def log(level: str, msg: str):
    """Print log with timestamp and level"""
    ts = datetime.now().strftime('%H:%M:%S')
    print(f"[{ts}] [{level}] {msg}")


def log_email(email: str, level: str, msg: str):
    """Print log with email in format like original"""
    ts = datetime.now().strftime('%H:%M:%S')
    print(f"[{ts}] [{level}] [{email}] {msg}")
...
def login(email: str, password: str) -> str:
    ...
    if not m:
        ...
        if 'jsdisabled' in body:
            raise Exception('Cần verify (JS disabled)')
...
def do_check_account(email: str, password: str, domain: str, api: str, auto_enable: bool, auto_create: bool) -> Dict:
    """Check one account - detailed logging like original"""
    result = {
    ...
    try:
        log_email(email, 'INFO', 'Login...')
        token = login(email, password)
        log_email(email, 'INFO', f'Login OK, listing rules...')
        ...
        if disabled:
            log_email(email, 'WARN', f'{len(disabled)} rule bị tắt: {", ".join(n for _, n in disabled)}')
            if auto_enable:
                for rid, name in disabled:
                    rp = enable_rule(token, rid, api)
                    if rp.status_code < 400:
                        result['fixed'] += 1
                        log_email(email, 'OK', f'Bật lại: {name}')
                    else:
                        log_email(email, 'ERROR', f'Bật fail: {name} [{rp.status_code}]')
        ...
        if auto_create:
            ...
            for rule_name, rule_def in NETFLIX_RULE_DEFS.items():
                if rule_name not in existing:
                    ...
                    if rp.status_code < 400:
                        result['created'] += 1
                        result['rules_enabled'] += 1
                        log_email(email, 'OK', f'Đã tạo: {rule_name}')
                    else:
                        log_email(email, 'ERROR', f'Tạo fail: {rule_name} [{rp.status_code}]')
        ...
        log_email(email, 'INFO', f'Trang thái: {result["status"]} | Rules: {result["rules_total"]} | Enabled: {result["rules_enabled"]} | Tạo mới: {result["created"]} | Bật lại: {result["fixed"]}')
...
def do_create_rules(email: str, password: str, rules: List[str], domain: str, api: str) -> Dict:
    ...
    try:
        ...
        for rule in rule_list:
            ...
            if r.status_code < 400:
                result['rules_ok'] += 1
                log_email(email, 'OK', f'Rule OK: {rule["displayName"]} [{r.status_code}]')
            else:
                log_email(email, 'ERROR', f'Rule fail: {rule["displayName"]} [{r.status_code}] {r.text[:120]}')
        ...
        log_email(email, 'INFO', f'Tạo xong: {result["rules_ok"]}/{result["rules_total"]} rules | Status: {result["status"]}')
    ...
    broadcast_ws({'type': 'create_result', 'data': result})
    return result
...
async def start_telegram_bot():
    ...
    async def status_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        no_rules = sum(1 for a in accounts if a.get('status') == 'NO_RULES')
        ...
        await update.message.reply_text(
            ...
            f"✅ OK: {ok}\n"
            f"⚠️ Partial/Fixed: {partial}\n"
            f"❌ Failed (Login): {failed}\n"
            f"○ No Rules: {no_rules}\n"
            f"🕐 Last Check: {last}",
            parse_mode="Markdown")
    ...
    async def accounts_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        for a in accounts[:20]:
            status_icon = "✅" if a.get('status') == 'OK' else "❌" if a.get('status') == 'LOGIN_FAILED' else "⚠️"
            rules_enabled = a.get('rules_enabled', 0)
            rules_total = a.get('rules_total', 0)
            text += f"{status_icon} {a['email']} ({rules_enabled}/{rules_total} rules)\n"
            if a.get('status') == 'LOGIN_FAILED':
                text += f"   └ Error: {a.get('error', 'N/A')[:50]}\n"
    ...
    async def check_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        # Check specific email or all
        if ctx.args:
            email = ctx.args[0]
            acc = next((a for a in accounts if a['email'].lower() == email.lower()), None)
            if not acc:
                await update.message.reply_text(f"❌ Account not found: {email}")
                return
            accounts = [acc]
            await update.message.reply_text(f"🔍 Checking {email}...")
        else:
            await update.message.reply_text(f"🔍 Checking {len(accounts)} account(s)...\n\nThis may take a while. Results will appear in logs.")

        def run():
            results = []
            for acc in accounts:
                result = do_check_account(acc['email'], acc['password'], DEFAULT_DOMAIN, 'outlook', True, True)
                results.append(result)

            # Send summary to Telegram
            ok = sum(1 for r in results if r['status'] == 'OK')
            partial = sum(1 for r in results if r['status'] in ('PARTIAL', 'FIXED'))
            failed = sum(1 for r in results if r['status'] == 'LOGIN_FAILED')
            created = sum(r.get('created', 0) for r in results)
            fixed = sum(r.get('fixed', 0) for r in results)
            ...
            # Try to send summary (may fail if update expired)
            try:
                import asyncio
                asyncio.run(update.message.reply_text(summary, parse_mode="Markdown"))
            except Exception:
                pass
    ...
    async def add_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            log_email(email, 'INFO', 'Added via Telegram')
            await update.message.reply_text(f"✅ Added: {email}")
            broadcast_ws({'type': 'account_added', 'email': email})
        except Exception as e:
    ...
    async def delete_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        log('INFO', f'Deleted via Telegram: {email}')
        await update.message.reply_text(f"🗑️ Deleted: {email}")
        broadcast_ws({'type': 'account_deleted', 'email': email})

    async def rules_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        text = "📜 *Available Rules*\n\n"
        for i, name in enumerate(NETFLIX_RULE_DEFS.keys(), 1):
            text += f"{i}. {name}\n"
        text += f"\nTotal: {len(NETFLIX_RULE_DEFS)} rules"
        await update.message.reply_text(text, parse_mode="Markdown")
...
async def add_account(account: AccountCreate):
    ...
    log_email(account.email, 'INFO', 'Added via API')
    broadcast_ws({'type': 'account_added', 'email': account.email})
...
async def batch_add_accounts(batch: AccountBatchCreate):
    ...
    for line in batch.accounts:
        ...
        if ':' in line:
            ...
            AccountStore.upsert(email.strip(), password.strip())
            added += 1
    log('INFO', f'Batch added {added} accounts via API')
    broadcast_ws({'type': 'accounts_updated'})
    return {"added": added}
...
async def delete_account(email: str):
    AccountStore.remove([email])
    log('INFO', f'Deleted via API: {email}')
    broadcast_ws({'type': 'account_deleted', 'email': email})
    return {"status": "ok"}
...
async def check_accounts(req: CheckRequest):
    ...
    log('INFO', f'Starting check for {len(accounts)} account(s)...')
...
async def create_rules(req: CreateRulesRequest):
    ...
    log('INFO', f'Starting create rules for {len(parsed)} account(s)...')
...
@app.get("/api/rules")
async def get_rules():
    return [{"name": name, **defs} for name, defs in NETFLIX_RULE_DEFS.items()]
